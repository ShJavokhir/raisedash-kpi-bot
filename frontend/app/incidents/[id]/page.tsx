...
import { formatDate, formatIncidentStatus, calculateDuration } from '@/lib/utils';
import { Clock, User, CheckCircle, AlertCircle, ExternalLink } from 'lucide-react';
...
export default function PublicIncidentPage() {
  ...
  const fetchIncidentDetail = async () => {
    try {
      const response = await fetch(`/api/public/incidents/${params.id}`);
      if (response.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-paper bg-noise flex items-center justify-center">
            ...
            <div className="tech-border bg-white p-4">
              ...
              <dl className="space-y-3">
                <div className="py-2 tech-border-b">
                  ...
                  <dd className="text-xs text-neutral-900">{incident.department_name || 'Unassigned'}</dd>
                </div>
                <div className="py-2 tech-border-b">
                  <dt className="text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-1">Group</dt>
                  <dd className="text-xs text-neutral-900">{incident.group_name}</dd>
                </div>
